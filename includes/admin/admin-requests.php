...
function twispay_tw_main_action()
{
    ...
    if (isset($_REQUEST['tw_general_action'])) {
        ...
        if (!current_user_can('administrator')) {
            wp_die(__('You do not have permission to access this file', 'twispay'), __('Error', 'twispay'), ['response' => 403]);
        }

        // Check if you are viewing the WordPress Administration Panels
        if (!is_admin()) {
            wp_die(__('You do not have permission to access the file from here', 'twispay'), __('Error', 'twispay'), ['response' => 403]);
        }
