        ...
        class WC_Gateway_Twispay_Gateway extends WC_Payment_Gateway
        {
            ...
            public function is_available()
            {
                ...
                if (WC()->cart && WC()->cart->needs_shipping()) {
                    $needs_shipping = true;
                } else if (is_page(wc_get_page_id('checkout')) && 0 < get_query_var('order-pay')) {
                ...
                if (!empty($this->enable_for_methods) && $needs_shipping) {
                    ...
                    } else if (empty($chosen_shipping_methods) || sizeof($chosen_shipping_methods) > 1) {
                        $check_method = false;
                    } else if (sizeof($chosen_shipping_methods) == 1) {
            ...
            function process_payment($order_id)
            {
                ...
                if (class_exists('WC_Subscriptions') && (true == wcs_order_contains_subscription($order_id))) {
                    ...
                    return ['result' => 'success', 'redirect' => plugin_dir_url(__FILE__) . 'twispay-subscription-processor.php?order_id=' . $order_id];
                } else {
                    /* Redirect to file that processes the purchase payments requests. */
                    return ['result' => 'success', 'redirect' => plugin_dir_url(__FILE__) . 'twispay-processor.php?order_id=' . $order_id];
                }
            }
            ...
            function process_refund($order_id, $amount = null, $reason = '')
            {
                ...
                $args = ['method' => 'DELETE', 'headers' => ['accept' => 'application/json', 'Authorization' => $apiKey]];
...
function twispay_tw_unhook_woo_order_emails($email_class)
{
    ...
    remove_action('woocommerce_order_status_pending_to_on-hold_notification', [$email_class->emails['WC_Email_Customer_Processing_Order'], 'trigger']);
